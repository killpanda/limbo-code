...
Single column: status bar (state/elapsed/tokens/model/workdir/queued + rainbow 🎯GOAL badge while a goal loop runs) → scrolling chat flow (user `❯`, streaming Markdown, tool cards, errors) → input box (Enter submits, Shift+Enter newline, paste markers, `ctrl+v` image attach) → hint line. Slash commands: `/sessions /new /export /compact /model /help /2048 /goal` + skill commands. '!'-leading input runs bash directly via a private BashTool (never touches history/LLM; the card is non-agent-owned so turn interrupts skip it). '/'-leading input that matches no command but looks like a path is sent as a normal message with existing files auto-attached (`ui/path_input.py`); a leading space forces plain text (covers both '/' and '!'). After palette changes run `python scripts/check_contrast.py` (≥ 4.5:1).
