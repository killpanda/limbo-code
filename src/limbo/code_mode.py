...
RUN_CODE_INSTRUCTIONS = (
    ...
    "program:\n\n"
    "- Batch, don't drip-feed: one program should complete a whole step of "
    "the task — combine as many `tools.*` calls as it takes (loop over "
    "files, branch on results, tolerate failures with `try/except`). That "
    "is the point of `run_code`: several tool calls in one LLM round "
    "trip. A program that merely wraps a single tool call without "
    "processing its output wastes the round trip.\n"
