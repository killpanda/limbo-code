...
@pytest.mark.asyncio
async def test_user_message_paths_grant_access(tmp_path):
    """Paths in a human-submitted message widen the fence, with a notice."""
    ...
    async with app.run_test() as pilot:
        ...
        input_widget = main_screen.query_one("#input", InputWidget)
        input_widget.text = f"帮我看看 {outside} 这个项目"
        input_widget.action_submit()
        await pilot.pause()
        await pilot.pause()

        assert outside.resolve() in main_screen.agent.registry.allowed_roots
        chat = main_screen.query_one("#chat", ChatWidget)
        assert "已允许访问" in chat.transcript_text()
        # The grant persisted into the saved session meta.
        meta = main_screen.agent.session_meta
        assert str(outside.resolve()) in meta.allowed_roots


@pytest.mark.asyncio
async def test_auto_grant_disabled_by_config(tmp_path):
    outside = tmp_path / "outside"
    outside.mkdir()
    workdir = tmp_path / "workdir"
    workdir.mkdir()
    ...
    cfg = Config()
    cfg.llm.api_key = "test"
    cfg.safety.auto_grant_user_paths = False
    fake_llm = FakeLLMClient([[TextChunk(text="ok")]])
    app = LimboApp(
        workdir=workdir,
        config=cfg,
    ...
    async with app.run_test() as pilot:
        ...
        input_widget = main_screen.query_one("#input", InputWidget)
        input_widget.text = f"帮我看看 {outside}"
        input_widget.action_submit()
        await pilot.pause()
        await pilot.pause()

        assert main_screen.agent.registry.allowed_roots == set()
